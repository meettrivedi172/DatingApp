...
onlineUsers$ = this.onlineUsersSource.asObservable()
  ...
  constructor(private tosts:ToastrService,private router:Router ) { }
   ...
   createHubConnection(user:User){
     ...
     this.hubConnection.on('UserIsOffnline',username=>{
      this.onlineUsers$.pipe(take(1)).subscribe({
        next:usernames=>{
     ...
     this.hubConnection.on('NewMessageReceived',({username,knownAs})=>{
      this.tosts.info(knownAs+ 'has sent you a new message! Click me to see it').onTap.pipe(take(1)).subscribe({
        next:() =>{
